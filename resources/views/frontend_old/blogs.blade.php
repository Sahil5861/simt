                    ...
                    <div class="container mt-4">
                        @if ($blogs->isNotEmpty())                
                            @foreach ($blogs as $blog)                
                                <div class="card2 shadow-lg border-0">
                                    <div class="row g-0">                    
                                        <!-- Image Section -->
                                        <div class="col-md-4">
                                            <a href="{{ route('blogs.details', $blog->id) }}">
                                                <img class="card-img" src="{{ asset($blog->image) }}" class="img-fluid rounded-end" alt="Blog Image">
                                            </a>
                                        </div>                                                
                                        <!-- Content Section -->
                                        <div class="col-md-8">
                                            <div class="card2-body">
                                                <h3 class="card-title" style="text-align:left; margin:0 10px;">{{ $blog->blog_title }}</h3>
                                                <p class="text-muted" style="text-align:left;">
                                                    <i style="margin:0 10px;" class="icon icon-calendar"></i>{{ $blog->created_at->format('d M Y') }}
                                                </p>
                                                <br>
                                                <p style="text-align: left;">
                                                    {!! trim($blog->meta_description)!!}
                                                </p>
                                            </div>
                                        </div>
                                    </div>
